<template>
  <div class="tab_class">
    ...
    <van-button type="primary" size="small" class="btn" @click="publish">发布</van-button>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    // 发布
    publish() {
      let addProduct = JSON.parse(JSON.stringify(this.addProduct))
      if (addProduct.name === '' || addProduct.detail === '' || addProduct.price === '' || addProduct.category === ''
        ...
        addProduct.price = addProduct.price * 100
        let params = {
          url: 'product/add',
          data: addProduct
        }
        api.post(params).then((res) => {
          if (res.code === 0) {
            Toast({ position: 'top', message: '发布成功喽' })
